...
namespace Finegamedesign.Utils
{
	public sealed class TestProgress
	{
		...
		public void Add()
		{
			Progress progress = new Progress();
			progress.levelMax = 3;
			Assert.AreEqual(0, progress.level);
			for (int cycle = 0; cycle < 4; cycle++)
			{
				Assert.AreEqual(1, progress.Add());
				Assert.AreEqual(2, progress.Add());
				Assert.AreEqual(0, progress.Add());
			}
		}

		[Test]
		public void GetLevelNormalDoesNotChangeAdd()
		{
			Progress progress = new Progress();
			progress.levelNormalMax = 1000;
			progress.level = 1;
